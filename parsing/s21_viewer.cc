...
bool s21::Model::s21_count_v_f(std::string file_name, data_t *obj) {  // открыли и посчитали, сколько нам потребуется памяти
  std::ifstream text;
  std::string ch;
...
s21::unint s21::Model::s21_Fconnect(data_t *obj, std::string ch, unint index_f) {
...
void s21::Model::s21_rotate(double **vertex, char xyz, double angle, unint count_v) {
  double temp_x = 0.0;
  double temp_y = 0.0;
  double temp_z = 0.0;
  switch (xyz) {
...
void s21::Model::s21_moveX(double **vertex, double move_x, unint count_v) {
  static double diff = 0;
  for (unint i = 0; i < count_v * 3; i += 3) (*vertex)[i] += move_x - diff;
  diff = move_x;
}

void s21::Model::s21_moveY(double **vertex, double move_y, unint count_v) {
  static double diff = 0;
  for (unint i = 1; i < count_v * 3; i += 3) (*vertex)[i] += move_y - diff;
  diff = move_y;
}

void s21::Model::s21_moveZ(double **vertex, double move_z, unint count_v) {
  static double diff = 0;
  for (unint i = 2; i < count_v * 3; i += 3) (*vertex)[i] += move_z - diff;
  diff = move_z;
}

void s21::Model::s21_scale(double **vertex, float scale, unint count_v) {
  if (scale == 0.0) return;
