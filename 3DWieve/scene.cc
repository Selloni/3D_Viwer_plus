...
void Scene::readFile(std::string path_file) {
  freeMem();
  int err_flag = 1;
  err_flag = controller.FasadeStr(path_file);  // fasa(pat_file);
  if (err_flag) {
    QMessageBox msgBox;
    msgBox.setText("The file was not considered");
    msgBox.exec();
  } else {
    qVertexes = controller.GetArrVertex();
    qFacets = controller.GetArrFacets();
  }
}
...
void Scene::draw() {
  if (controller.GetCountFacets() > 3) {
    glVertexPointer(3, GL_DOUBLE, 0, controller.GetArrVertex());
    glEnableClientState(GL_VERTEX_ARRAY);
    veretexStile(vS);
    vertexColor(vC);
    if (vS != 0) {
      glPointSize(vW);  // size point
      glDrawArrays(GL_POINTS, 0, controller.GetCountVertex());
    }
    lineColor(lC);
    lineStyle(lS);
    glDrawElements(GL_LINES, (controller.GetCountFacets() * 2), GL_UNSIGNED_INT,
                   controller.GetArrFacets());
